package adventofcode.aoc2024.day5;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (ss[a].contains("|")) {
            rules.add(new Pair(
                    Integer.parseInt(ss[a].split("\\|")[0]),
                    Integer.parseInt(ss[a].split("\\|")[1]))
            );
            a++;
        }
        int sum = 0;
        while (a < ss.length) {
            ...
            for (int i = 0; i < t.length; i++) {
                pages.add(Integer.parseInt(t[i]));
            }
            if (checkList(pages, rules)) {
                sum += pages.get(t.length / 2);
            }
            a++;
        }
    ...
    private static boolean checkList(List<Integer> pages, List<Pair<Integer, Integer>> rules) {
        for (int i = 0; i < pages.size() - 1; i++) {
            for (int j = i; j < pages.size(); j++) {
                for (int k = 0; k < rules.size(); k++) {
                    if (
                            rules.get(k).getSecond().equals(pages.get(i)) &&
                            rules.get(k).getFirst().equals(pages.get(j))
                        )
                    {
                        return false;
                    }
