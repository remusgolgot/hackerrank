...
public class Main2 {
    ...
    static String s = "afg afdbec cbdgaf afcgedb bedgf cdagef ga afcdb gbca gfbda | cfagdb edcafg cbfgda bgfed\n" +
    ...
    // len = 3 -> 7
    // len = 7 -> 8
    // len = 6 -> 0,9,6   ( 9 contained by both 4 and 7 ), (6 contained by neither) else it's a 0 (contained by just 7)
    // len = 5 -> 2,3,5   ( 3 has 1-4 commons ), 5 fully filled by 9 and 6
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < ss.length; i++) {
            String nrs[] = new String[10]; // the numbers, from 0 to 9
            String t = ss[i].split("\\|")[0];
            String[] u = t.trim().split(" ");
            for (int j = 0; j < u.length; j++) {
                if (u[j].length() == 2) {  // one
                    nrs[1] = u[j];
                }
                if (u[j].length() == 3) {  // seven
                    nrs[7] = u[j];
                }
                if (u[j].length() == 4) {  // four
                    nrs[4] = u[j];
                }
                if (u[j].length() == 7) {  // eight
                    nrs[8] = u[j];
                }
            }
            String s1 = nrs[1];
            String s2 = nrs[4];
            StringBuilder oneFourCommon = new StringBuilder();
            for (int j = 0; j < s1.length(); j++) {
                if (s2.contains(s1.substring(j, j + 1))) {
                    oneFourCommon.append(s1, j, j + 1);
                }
            }
            for (int j = 0; j < u.length; j++) {
                boolean not4 = false, not7 = false;
                if (u[j].length() == 6) {
                    String s4 = nrs[4];
                    String s7 = nrs[7];
                    for (int l = 0; l < s4.length(); l++) {
                        if (!u[j].contains(s4.substring(l, l + 1))) {
                            not4 = true;
                            break;
                        }
                    }
                    for (int l = 0; l < s7.length(); l++) {
                        if (!u[j].contains(s7.substring(l, l + 1))) {
                            not7 = true;
                            break;
                        }
                    }
            ...
            for (int j = 0; j < u.length; j++) {
                if (u[j].length() == 5) {
                    String c1 = oneFourCommon.substring(0, 1);
                    String c2 = oneFourCommon.substring(1, 2);
                    if (u[j].contains(c1) && u[j].contains(c2)) {
                        nrs[3] = u[j];
                        continue;
                    }
                    String s9 = nrs[9];
                    ...
                    for (int l = 0; l < u[j].length(); l++) {
                        if (!s9.contains(u[j].substring(l, l + 1))) {
                            not9 = true;
                            break;
                        }
                    }
            ...
            String t1 = ss[i].split("\\|")[1];
            String t2[] = t1.trim().split(" ");
            int number = 0;
            for (int j = 0; j < t2.length; j++) {
                int num = 0;
                for (int l = 0; l <= 9; l++) {
                    if (hasSameChars(t2[j], nrs[l])) {
                        num = l;
                        break;
                    }
                }
